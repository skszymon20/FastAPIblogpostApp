...
async def create_post(post: PostCreate, db: Annotated[AsyncSession, Depends(get_db)]):
    ...
    db.add(new_post)
    await db.commit()
    await db.refresh(new_post, attribute_names=["author"])  # Refresh the post to include the author relationship
    return new_post
...
async def update_post(post_id: int, post_data: PostCreate, db: Annotated[AsyncSession, Depends(get_db)]):
    ...
    await db.commit()
    await db.refresh(post, attribute_names=["author"])  # Refresh the post to include the author relationship
    return post
...
async def general_http_exception_handler(request: Request, exc: StarletteHTTPException):
    ...
    message = (
        exc.detail if exc.detail else "An error occurred. Please check your request and try again."
    )
    return templates.TemplateResponse(
        request,
        "error.html",
